...
def doBinaryAddition(pBits: List[Boolean], qBits: List[Boolean], carryBit: Boolean): List[Boolean] = {
  smallList(pBits, qBits).isEmpty match {
    case true => finishBinaryAdd(bigList(pBits, qBits), carryBit)
...
def smallList(pList: List[Boolean], qList: List[Boolean]): List[Boolean] = {
  (pList.length).equals(qList.length) match {
    case true => pList
    case false => {
      qList.length > pList.length match {
        case true => pList
        case false => qList
      }
    }
...
def bigList(pList: List[Boolean], qList: List[Boolean]): List[Boolean] = {
  (pList.length).equals(qList.length) match {
    case true => pList
    case false => {
      qList.length > pList.length match {
